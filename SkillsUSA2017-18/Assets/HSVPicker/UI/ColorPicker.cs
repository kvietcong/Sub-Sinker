...
public class ColorPicker : MonoBehaviour
{
    ...
    private void Update()
    {
        if(Input.GetKeyDown(KeyCode.Return))
        {
            if(color==0)
            {
                GameManager.instance.playerSettings.PlayerPrimaryColor = CurrentColor;
                gameObject.SetActive(false);
            }

            if (color == 1)
            {
                GameManager.instance.playerSettings.PlayerSecondaryColor = CurrentColor;
                gameObject.SetActive(false);
            }

            if (color == 2)
            {
                GameManager.instance.playerSettings.PlayerDecorColor = CurrentColor;
                gameObject.SetActive(false);
            }
        }
